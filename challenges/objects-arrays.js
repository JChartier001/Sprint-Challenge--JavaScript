...
const contactInfo = [];
graduates.map(function (currentValue, item) {
...
const unisWithUni = [];
for (uni in graduates) {
    if (graduates[uni].university.match("Uni")) {
        unisWithUni.push(graduates[uni].university);
    }
}
...
zooAnimals.forEach(function (item) {
    populationTotal.push(item.population);
})
...
*/
const populationTotal1 = [];
zooAnimals.forEach(function (item) {
    populationTotal1.push(item.population);
})

const total1 = zooAnimals.reduce((accumulator, item) => {
    return accumulator + item.population;
}, 0);
console.log(total1);
